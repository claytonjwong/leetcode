...
namespace TopDownBruteForce {
    class Solution {
        ...
        int go(int i) {
            ...
            return A[i] + min(go(i + 1), go(i + 2)); // cost of i-th stair plus min(one step or two steps) 🎯
        }
    public:
        int minCostClimbingStairs(VI& A_) {
            A = A_;
            N = A.size();
            return min(go(0), go(1)); // start on min(first step, second step)
        }
    };
}
...
namespace TopDownMemo {
    class Solution {
        ...
        int go(int i, Map&& m = {}) {
            ...
            return m[i] = A[i] + min(go(i + 1, move(m)), go(i + 2, move(m))); // cost of i-th stair plus min(one step or two steps) 🎯
        }
    public:
        int minCostClimbingStairs(VI& A_) {
            A = A_;
...
namespace BottomUp {
    ...
    public:
        ...
        int minCostClimbingStairs(VI &A) {
            ...
            for (auto i{2}; i < N; ++i)
                dp[i] = A[i] + min(dp[i - 2], dp[i - 1]); // cost of i-th stair plus min(one step or two steps) 🎯
    ...
    public:
        ...
        int minCostClimbingStairs(VI& A) {
            ...
            for (auto i{ 2 }; i < N; ++i, a = b, b = c)
                c = A[i] + min(a, b); // cost of i-th stair plus min(one step or two steps) 🎯
